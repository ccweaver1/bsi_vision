...
def get_symbol(num_outputs=9, output_name="reg_out", dtype='float32', **kwargs):
    data = mx.sym.Variable('data')
    output_label = mx.sym.Variable(output_name)
    if dtype == 'float32':
    ...
    reg_fc = mx.sym.FullyConnected(data=flatten, num_hidden=num_outputs, name='reg_fc', no_bias=True)
    lro = mx.sym.LinearRegressionOutput(data=reg_fc, label=output_label, name=output_name)
    # reg_out = mx.sym.Activation(data=fc1, act_type='linear', name='reg_out')
    # softmax = mx.sym.SoftmaxOutput(data=fc1, name='softmax')
    # we want regression output, not softmax
    return lro
